...
fun DrawScope.drawArcPreviewRadiusPudorys(
    state: MongeState,
    snappedPointLogical: Offset?
) {
    val center = state.arc.arcCenterPudorys ?: return
    if (state.arc.arcRadiusPointPudorys != null) return
    ...
    val radius = hypot(cursorLogical.x - center.x, cursorLogical.y - center.y)
...
fun DrawScope.drawArcPreviewEndPudorys(
    state: MongeState,
    snappedPointLogical: Offset?
) {
    val center = state.arc.arcCenterPudorys ?: return
    val radiusPoint = state.arc.arcRadiusPointPudorys ?: return
    ...
    // ✅ 1) Šedá kružnice po zadání poloměru
    drawArc(
        color = Color.Gray,
        startAngle = 0f,
        sweepAngle = 360f,
    ...
    val start = state.arc.arcStartPointPudorys ?: return

    val dx1 = start.x - center.x
    val dy1 = start.y - center.y
    val dx2 = cursorLogical.x - center.x
    val dy2 = cursorLogical.y - center.y

    val angleStartRad = atan2(dy1, dx1)
    val angleEndRad = atan2(dy2, dx2)
