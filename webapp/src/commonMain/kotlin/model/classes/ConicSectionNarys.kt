...
private val SUBSCRIPT_REGEX = Regex("[₀₁₂₃₄₅₆₇₈₉]")
fun String.removeAllSubscripts(): String =
    replace(SUBSCRIPT_REGEX, "")
data class CircleGeomXZ(val x0: Float, val z0: Float, val r: Float)
fun ConicSectionNarys.tryAsCircleGeomXZ(
    epsRel: Float = 1e-4f,
    epsAbs: Float = 1e-6f
): CircleGeomXZ? {
    val a = this.a
    ...
    val x0 = -d / (2f * a)
    val z0 = -e / (2f * a)
    val r2 = x0 * x0 + z0 * z0 - f / a
    if (r2 <= 0f) return null
    return CircleGeomXZ(x0, z0, sqrt(r2))
}
data class ConicInputHyperbolaNarys(
    val vertex: Offset,
    val axis: Offset,
    val line1: NamedLineNarys,
    val line2: NamedLineNarys,
    )
