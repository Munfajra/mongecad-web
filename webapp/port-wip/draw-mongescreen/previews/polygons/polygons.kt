...
import monge.input.planeobjects.conicsections.liftNarysToPlane
import monge.input.planeobjects.conicsections.liftPudorysToPlane
...
fun DrawScope.drawRegularPolygonPreview(
    ...
    when (state.projectionMode) {
        ...
        ProjectionMode.AXO -> {
            if (state.drawobjects != Mongeobjects.REGULAR_POLYGON_IN_PLANE) return
            if (state.projectionPhase != "rp_vertex_axo") return

            val basis = state.basis ?: return
            val plane3D = state.selectedPlaneForCircle ?: state.selectedPlanes.firstOrNull() ?: return
            val eq = plane3D.equation ?: return
            val centerAxo = state.pendingPoint1 ?: return
            val cursorAxo = snappedPointLogical ?: screenToAxoOverlayLocal(state.cursorPosition, state, basis)

            val center3D = liftAxoOverlayPointToPlane(centerAxo, plane3D, state) ?: return
            val vertex3D = liftAxoOverlayPointToPlane(cursorAxo, plane3D, state) ?: return

            val dx = vertex3D.x - center3D.x
            val dy = vertex3D.y - center3D.y
            val dz = vertex3D.z - center3D.z
            if (dx * dx + dy * dy + dz * dz < 1e-8f) return
            ...
            val vertsAxo = verts3D.map {
                projectPoint3DToAxoLocal(Offset3D(it.x, it.y, it.z), basis)
            }
            drawRedCrossAxo(centerAxo, state)
            drawPolygonOutlinePreviewDashedAxo(vertsAxo, state)
        }
