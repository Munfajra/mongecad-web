...
fun handleRegularPolygonInPlaneClickAxo(state: MongeState, logical: Offset) {
    if (state.drawobjects != Mongeobjects.REGULAR_POLYGON_IN_PLANE) return

    when (state.projectionPhase) {
        "", "pudorys_start", "narys_start", "bokorys_start", "axo_start" -> {
            val plane = state.selectedPlaneForCircle ?: state.selectedPlanes.firstOrNull()
            if (plane == null) {
            ...
            setProjectionPhase("rp_vertex_axo", state)
            println("➡️ Zvol AXO průmět VRCHOLU.")
        }
        ...
        "rp_vertex_axo" -> {
            val plane = state.selectedPlaneForCircle ?: state.selectedPlanes.firstOrNull() ?: run {
                println("❗Rovina nenalezena.")
                setProjectionPhase("pudorys_start", state)
                return
            }
            val centerAxo = state.pendingPoint1 ?: run {
                println("❗Chybí první klik (střed).")
                setProjectionPhase("pudorys_start", state)
                return
            }
            val center3D = liftAxoOverlayPointToPlane(centerAxo, plane, state) ?: run {
                println("❗AXO průmět středu nelze jednoznačně zvednout do roviny ${plane.name}.")
                setProjectionPhase("pudorys_start", state)
                return
            }
            val vertex3D = liftAxoOverlayPointToPlane(logical, plane, state) ?: run {
                println("❗AXO průmět vrcholu nelze jednoznačně zvednout do roviny ${plane.name}.")
                setProjectionPhase("pudorys_start", state)
                return
            }

            val delta = Vec3(
                vertex3D.x - center3D.x,
                vertex3D.y - center3D.y,
                vertex3D.z - center3D.z
            )
            if (delta.norm() < 1e-6f) {
            ...
            val normal = planeEquationFromPlane3D(plane).normal().normalize()
            val style = state.currentLineStyleSettings
            buildRegularPolygonWithAxoProjections(
                state = state,
                center = Vec3(center3D.x, center3D.y, center3D.z),
                vertex0 = Vec3(vertex3D.x, vertex3D.y, vertex3D.z),
                n = n,
                normal = normal,
                baseName = "P",
                planeId = plane.id,
            ...
            repeatCons(state)
            state.triggerRedraw++
            println("✅ Vytvořen pravidelný $n-úhelník v rovině ${plane.name} z AXO průmětu.")
        }
    }
}
